...
export default function useProfile() {
  ...
  const updateProfile = async (param) => {
    try {
      ...
      await axios.post(
        `/api/profiles/${param.username}`, formData,
        config
      )
  ...
  const changePassword = async (token, formData) => {
    try {
      ...
      const res = await axios.post(`/api/change_password`, formData, config)
  ...
  const searchProfile = async (search) => {
    try {
      const res = await axios.get(`/api/profiles/search/${search}`)
